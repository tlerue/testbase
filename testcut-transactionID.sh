...
chunksize=19288
#converison base64
openssl base64 -in entretienconseil.ipa -out test.txt
fileglobal=( `cat "entretienconseil.ipa" `)
fileSize=${#fileglobal[@]}
#decoupage
split -b $chunksize test.txt data.tsv
index=1
...
for csv in $(ls data.tsv*); do
	...
	if [ $index = 1 ]; then
		echo "<InternalAppChunk xmlns='http://www.air-watch.com/servicemodel/resources'><ChunkData>${filepartcontent[@]}</ChunkData><TransactionId></TransactionId><ChunkSequenceNumber>$index</ChunkSequenceNumber><TotalApplicationSize>1928856</TotalApplicationSize><ChunkSize>19288</ChunkSize><IsAssembled>0</IsAssembled></InternalAppChunk>" > la$index
		#requete CURL
		...
		echo "<InternalAppChunk xmlns='http://www.air-watch.com/servicemodel/resources'><TransactionId>$transcationId</TransactionId><ChunkData>${filepartcontent[@]}</ChunkData><ChunkSequenceNumber>$index</ChunkSequenceNumber><TotalApplicationSize>1928856</TotalApplicationSize><ChunkSize>19288</ChunkSize><IsAssembled>0</IsAssembled></InternalAppChunk>" > la$index
		#requete CURL
		response=$(curl -k -H "Content-type: application/xml" -H "Authorization: Basic YWRtaXRlbXVuYjpKYW52aWVyMjAxNSo=" -H "aw-tenant-code: 1ONPA4AAAAG6A53QADQA" -X POST "http://a-ibmobe00.srv-ib.ibp/API/v1/mam/apps/internal/uploadchunk" --data-binary @la$index)	
		echo $response	
	fi
